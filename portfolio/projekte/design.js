...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('download-btn')?.addEventListener('click', (e) => {
        ...
        if (item) {
            // Download PDF if type is pdf, else image
            if (item.type === 'pdf' && item.pdfUrl) {
                downloadPhoto(item.pdfUrl);
            } else {
                downloadPhoto(item.imageUrl);
            }
        }
    });
...
function updateDetailContent(item) {
    const isPdf = item.type === 'pdf' && item.pdfUrl;
                ...
                <div class="pdf-container">
                     <object data="${item.pdfUrl}" type="application/pdf" width="100%" height="100%">
                        <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; height:100%; color:#555;">
                            <p style="margin-bottom:10px;">Vorschau nicht verfügbar.</p>
                            <a href="${item.pdfUrl}" target="_blank" class="download-link">PDF öffnen</a>
                        </div>
                     </object>
                </div>
            `;
            // Disable zoom for PDF (it handles its own zoom usually, or we disable pan logic/mess)
            // Actually let's just reset transform logic for PDF so it doesn't break iframe interaction
        } else {
...
function closeDetail() {
    if (detailScreen) {
        ...
        setTimeout(() => {
            ...
            // Check for iframes and clear them to stop processes?
...
function setupZoomPan() {
    ...
    zoomSlider.addEventListener('input', (e) => {
        const item = findImageById(activePhotoId);
        if (item && item.type === 'pdf') return; // Disable custom zoom for PDF
    ...
    // Attach listeners
    // Note: We might want to remove these listeners for PDF mode if they interfere, but check inside works too
...
function loadImagesFromData(items, containerId) {
    ...
    if (items.length === 0) {
        ...
        container.innerHTML = '<p style="text-align:center; color:#999; grid-column:1/-1;">Keine Projekte in dieser Kategorie.</p>';
        return;
    }
    ...
    items.forEach(item => {
        ...
        thumb.onerror = () => {
            ...
            thumb.src = '../../images/kategorien/design.jpg'; // Fallback
            loadedCount++;
            if (loadedCount === items.length) {
                document.getElementById('loader')?.classList.remove('visible');
                relayoutGrid(container);
...
function renderFloatingPreview() {
    ...
    favoriteIds.forEach(id => {
        ...
        let url = data ? (data.type === 'pdf' ? data.imageUrl : data.imageUrl) : null;
        // Note: For PDF items we show the thumbnail image
...
function downloadFavoritesZip() {
    ...
    const zip = new JSZip();
    const folder = zip.folder("medien-vom-noe-design");
    ...
    favoriteIds.forEach(id => {
        ...
        let downloadUrl = (data.type === 'pdf' && data.pdfUrl) ? data.pdfUrl : data.imageUrl;
        ...
        if (processedCount === favoriteIds.length) {
            zip.generateAsync({ type: "blob" })
                .then(function (content) {
                    ...
                    link.download = "medien-vom-noe_design_selection.zip";
                    document.body.appendChild(link);
                    link.click();
                    document.body.removeChild(link);
