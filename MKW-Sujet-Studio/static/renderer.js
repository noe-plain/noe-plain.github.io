...
const textBlock=(b,key)=>MKWLayout.textLayout(b,measureTextWidth,key);
...
// The shadow follows the glyphs or logo silhouette, never the surrounding box.
function softShadow(ctx,draw,opacity=.28){ctx.save();ctx.globalCompositeOperation='multiply';ctx.globalAlpha*=opacity;ctx.shadowColor='#000';ctx.shadowBlur=18;ctx.shadowOffsetY=4;ctx.fillStyle='#00000001';draw();ctx.restore()}
function drawText(ctx,layout,opts={}){for(const e of layout.records){if(!e.visible||!e.text||(opts.only&&opts.only!==e.key)||opts.skip===e.key)continue;ctx.save();ctx.font=`${e.size}px ${fontFamily(e.role)}`;ctx.textBaseline='top';ctx.fillStyle=opts.color||e.color;ctx.textAlign=e.align;e.lines.forEach((t,i)=>ctx.fillText(t,e.x,e.y+i*e.size*1.15));ctx.restore()}}
...
async function paint(ctx,b,s=1,opts={}){
 ...
 if((!opts.only||opts.only==='textShadow')&&layout.records.some(e=>e.visible&&e.text))softShadow(ctx,()=>drawText(ctx,layout,{color:'#000'}),opts.only?1:.28);
 if((!opts.only||opts.only==='logoShadow')&&b.elements.logo.visible)softShadow(ctx,()=>drawLogo(ctx,b,logo),opts.only?1:.28);
 ctx.save();ctx.beginPath();ctx.rect(safe.x,safe.y,safe.w,safe.h);ctx.clip();
 drawText(ctx,layout,opts);
...
function syncOverlays(force=false){
 ...
 for(const b of p.boards){const root=document.querySelector(`.interaction[data-board="${b.id}"]`);if(!root)continue;const activeKey=step==='text'&&selected===b.id?layer:null,block=textBlock(b,activeKey),safe=block.safe;
  ...
  for(const el of root.querySelectorAll('.text-editor')){const e=block.records.find(r=>r.key===el.dataset.key),editing=editingText?.el===el;el.hidden=!e;if(!e)continue;if(!editing)el.textContent=b.elements[e.key].text;
   Object.assign(el.style,{left:'0px',top:(e.y-block.y)*s+'px',fontFamily:fontFamily(e.role),fontSize:e.size*s+'px',width:block.w*s+'px',height:e.h*s+'px',lineHeight:e.size*1.15*s+'px',textAlign:e.align,transform:'none',color:editing||!e.visible?e.color:'transparent',textShadow:'none',opacity:'1'});
  }
