...
async function layerBitmap(b,key){
 ...
 if(!['image','background'].includes(key)){
  let r;if(key==='textShadow')r=block;else if(key==='logoShadow'||key==='logo')r=MKWLayout.logoRect(b.elements.logo,logoAspects.get(b.id));else{const e=block.records.find(e=>e.key===key);r={x:block.x,y:e.y,w:block.w,h:e.h}}
  const fade=key.endsWith('Shadow')?30:2;left=Math.max(0,Math.floor(r.x-fade));top=Math.max(0,Math.floor(r.y-fade));w=Math.max(1,Math.min(b.w,Math.ceil(r.x+r.w+fade))-left);h=Math.max(1,Math.min(b.h,Math.ceil(r.y+r.h+fade))-top);
 }
...
async function buildPSD(progress=()=>{}){
 ...
 for(const [index,{b,left,top}] of layout.boards.entries()){
  ...
  for(const key of ['logo','logoShadow','copyright','subtitle','title','date','textShadow','image','background']){
   ...
   if(shadow){l.blendMode='multiply';l.opacity=.28}else if(e&&key!=='logo')l.text=textLayerData(e,left,top);
   if(key==='image'){
   ...
   if(MKWLayout.keys.includes(key)||key==='textShadow'){textChildren.push(l);if(key==='textShadow')children.push({name:'Text · gemeinsame Box',opened:true,children:textChildren.reverse()})}else children.push(l);
  }
