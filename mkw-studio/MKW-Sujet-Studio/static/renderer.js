...
function softShadow(ctx,draw,opacity=.28){ctx.save();ctx.globalCompositeOperation='multiply';ctx.globalAlpha*=opacity;ctx.shadowColor='#000';const transform=ctx.getTransform(),scale=Math.hypot(transform.a,transform.b);ctx.shadowBlur=125*scale;ctx.shadowOffsetY=4*scale;ctx.fillStyle='#00000001';draw();ctx.restore()}
