...
function render(){
 ...
 document.querySelectorAll('[data-family-background]').forEach(input=>input.onchange=()=>mutate(()=>setFamilyDesign(input.dataset.familyBackground,{background:input.value}),false));
...
function renderPanel(){
 ...
 if(step==='crop'){
  panel.innerHTML=`<div class="panel-row"><button data-fit="fill">Fläche füllen</button><button data-fit="fit">Einpassen</button><button data-fit="center">Zentrieren</button><label>Bild <input id="scale" aria-label="Bildskalierung in Prozent" type="number" min="0.1" step="0.1" value="${(b.image.scale*100).toFixed(1)}"> %</label><details><summary>Mehr</summary><div class="more-options"><button data-fit="original">Originalgrösse</button><button data-fit="reset">Zurücksetzen</button><label>Drehen <input id="angle" type="number" value="${b.image.angle}">°</label><button id="flipX">↔ Spiegeln</button><button id="flipY">↕ Spiegeln</button><label>Hintergrund <input id="bg" type="color" value="${b.family?.enabled?b.family.background:b.bg}"></label></div></details></div><p class="panel-caption">Bild ziehen. An einer Ecke skalieren. <strong>Das Seitenverhältnis bleibt erhalten.</strong></p>`;
