...
def capture_info(src):
    try:
        exif = src.getexif()
        value = next((exif.get(tag) for tag in (36867, 36868, 306) if exif.get(tag)), None)
        if not value: return ('','')
        value = value.decode(errors='ignore') if isinstance(value, bytes) else str(value)
        taken = datetime.strptime(value[:19], '%Y:%m:%d %H:%M:%S')
        return (taken.strftime('%d.%m.%y'), WEEKDAYS[taken.weekday()])
...
def normalize(raw):
    with Image.open(io.BytesIO(raw)) as src:
        ...
        capture_date, capture_day = capture_info(src)
        src.load()
        ...
        return dict(data='data:image/png;base64,' + base64.b64encode(out.getvalue()).decode(), width=im.width, height=im.height, warning=warning, captureDate=capture_date, captureDay=capture_day)
